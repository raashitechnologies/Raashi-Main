...
import {
  ...
  LogIn, User, LayoutDashboard, LogOut,
} from "lucide-react";
...
function UserMenu({ isScrolled, isDesktop, springTransition }: UserMenuProps) {
  ...
  // Header is always dark, so always use white text
  ...
  return (
    ...
    >
      <motion.button
        initial={false}
        animate={isDesktop ? {
          paddingLeft: isScrolled ? 8 : 14,
          paddingRight: isScrolled ? 8 : 14,
          fontSize: isScrolled ? "13px" : "14px"
        } : { paddingLeft: 12, paddingRight: 12, fontSize: "14px" }}
        transition={springTransition}
        className={`flex items-center gap-2 py-2 rounded-lg font-medium transition-colors ${btnClass}`}
        aria-haspopup="true"
        aria-expanded={open}
      >
        <div
          className={`w-7 h-7 rounded-full flex items-center justify-center ${isScrolled ? "bg-white/20" : "bg-brand-blue/10"
            }`}
        >
          <User size={14} className={isScrolled ? "text-white" : "text-brand-blue"} />
        </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-floating border border-brand-navy/[0.08] py-1.5 z-[200]"
          >
            <div className="px-3 py-2 border-b border-brand-navy/[0.06] mb-1">
              <p className="text-xs font-semibold text-brand-navy truncate">{user.name}</p>
              <p className="text-[10px] text-brand-navy/40 truncate">{user.email}</p>
              <span className="inline-block mt-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-brand-blue/10 text-brand-blue">
                {user.role}
              </span>
            </div>
            <Link
              to={user.role === "admin" ? "/admin/dashboard" : "/coordinator/dashboard"}
              className="flex items-center gap-2 px-3 py-2 text-sm text-brand-navy/70 hover:text-brand-navy hover:bg-[#F5F0E6] transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-red-600/70 hover:text-red-600 hover:bg-red-50 transition-colors w-full text-left"
            >
              <LogOut size={14} />
              Sign Out
            </button>
...
export function Header({
  ...
  const headerVariants = {
    ...
    scrolled: isDesktop ? {
      ...
      marginTop: "0px",
      borderRadius: "0px",
      backgroundColor: "rgba(23, 40, 94, 0.60)",
      ...
      <div className="fixed top-0 left-0 right-0 z-50 pointer-events-none">
        <motion.header
          ...
          style={{
            ...
            WebkitBackdropFilter:
              isDesktop || currentState === "scrolled"
        ...
        >
          ...
          >
            ...
            <div className={isDesktop ? "account-slot hidden xl:flex" : "hidden xl:flex items-center shrink-0"}>
              {isAuthenticated && user ? (
                <UserMenu isScrolled={scrolled} isDesktop={isDesktop} springTransition={springTransition} />
              ) : (
                // ghost-light variant renders white text on dark header background
        ...
        {drawerOpen && (
          <>
            ...
            <motion.nav
              ...
              className="fixed right-0 top-0 bottom-0 w-72 bg-white z-[70] xl:hidden flex flex-col shadow-floating"
            ...
            >
              ...
              {/* Auth section */}
              <div className="pt-2 border-t border-brand-navy/[0.06]">
                {isAuthenticated && user ? (
                  <>
                    <Link
                      to={user.role === "admin" ? "/admin/dashboard" : "/coordinator/dashboard"}
                      className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium text-brand-blue hover:bg-brand-blue/5 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium text-red-600/70 hover:bg-red-50 transition-colors w-full text-left"
                    >
                      <LogOut size={15} />
                      Sign Out
                    </button>
                  ...
                  <Link
                    to="/login"
                    className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium text-brand-navy/60 hover:bg-[#F5F0E6] transition-colors"
                  >
                    <LogIn size={15} />
                    Login
                  </Link>
