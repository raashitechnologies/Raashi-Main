...
export default function FoldText({
  ...
  const actualSplitBy = isMobile ? "word" : splitBy;
  const actualPerspective = isMobile ? "none" : perspective;
  ...
  return (
    ...
    >
      ...
      <span aria-hidden="true" className="inline" style={{ perspective: actualPerspective }}>
        {actualSplitBy === "word" ? (
...
function AnimatedSegment({
  ...
  const initialVariants = reducedMotion ? { opacity: 0 } : isMobile ? { opacity: 0, y: hinge === "top" ? -10 : 10 } : { opacity: 0, rotateX: initialRotate, y: hinge === "top" ? -5 : 5 };
  const animateVariants = shouldAnimate ? (reducedMotion ? { opacity: 1 } : isMobile ? { opacity: 1, y: 0 } : { opacity: 1, rotateX: 0, y: 0 }) : {};
