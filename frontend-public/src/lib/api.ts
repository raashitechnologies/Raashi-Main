...
// The local development proxy deliberately remains relative. Production must
// always address the Cloudflare Worker directly: Pages does not proxy /api.
 ...
 */
/** Build an API URL exactly once, without duplicate `/api/v1` segments. */
...
/**
 * Resolve API-relative assets returned by D1-backed endpoints. Public static
 * assets (for example `/logo.png`) intentionally remain frontend-relative.
 */
